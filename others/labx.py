...
def plot_hist(title, xlabel, ylabel, data, filename, bins, range, x_scale_log=False, y_scale_log=False, close=True,
              alpha=1, legend=0, bar=0, height=0):
    if x_scale_log:
        plt.xscale('log')
    if y_scale_log:
        plt.yscale('log')
    ...
    plt.ylabel(ylabel)
    if bar:
        hist = plt.bar(height, data, width=bar, alpha=alpha)
    else:
...
plot_plt("Znormalizowany histogram dla stóp zwrotu", 'wartość', 'częstość', (histStopyZwrotu[0] / StopySuma),
         "../10_ZnormalizowanyHistogramDlaStopZwrotu.png", y_scale_log=True)
plot_plt("Znormalizowany histogram dla white noise", 'wartość', 'częstość', (histWhiteNoise[0] / WhiteSuma),
         "../11_ZnormalizowanyHistogramDlaWhiteNoise.png", y_scale_log=True)
...
plot_hist("Histogramy dla stop zwrotu i white noise", 'wartość', 'częstośc', (histWhiteNoise[0] / WhiteSuma),
          "../12_znormalizowane histogramy(pologarytmiczna).png", bins=500, range=(-10, 10), y_scale_log=True, bar=0.1,
...
plot_hist("Histogramy dla stop zwrotu i white noise", 'wartość', 'częstośc', (histStopyZwrotu[0] / StopySuma),
          "../12_znormalizowane histogramy(pologarytmiczna).png", bins=500, range=(-10, 10), y_scale_log=True, bar=0.1,
          height=histWhiteNoise[1][:-1], alpha=0.5, legend=["white noise", "stopy zwrotu"])
...
acf_white_noise_abs = stats.tsa.acf(abs_white_noise,
                                    nlags=len(abs_white_noise) - 1)  # autokorelacja modulu bialego szumu
plt.ylim(-0.01, 0.01)
...
plt.ylim(-0.025, 0.05)
plot_plt("Autokorelacja modułu stóp zwrotu", 'przesunięcie', 'korelacja', acf_stopy_abs,
         "../15a_AutokorelacjaModuluStopZwrotu.png")

plt.ylim(-0.025, 0.2)
plot_plt("Autokorelacja modułu white noise", 'przesunięcie', 'korelacja', acf_white_noise_abs,
         "../16_AutokorelacjaModuluWhiteNoise.png")

plt.ylim(-0.025, 0.05)
plot_plt("Autokorelacja modułu white noise", 'przesunięcie', 'korelacja', acf_white_noise_abs,
         "../16a_AutokorelacjaModuluWhiteNoise.png")
